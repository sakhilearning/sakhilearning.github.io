...
var lastProvider='none',lastError=null,primeDone=false,browserAttempt=0,premiumCache={};
...
async function speakText(text){
  ...
  if(window.SakhiCloud&&SakhiCloud.state().configured){
    try{var bytes=premiumCache[text];if(!bytes){bytes=await SakhiCloud.speak(text);if(bytes&&bytes.byteLength>100)premiumCache[text]=bytes.slice(0);}if(bytes&&bytes.byteLength>100)return await playBytes(bytes.slice(0));throw new Error('Empty narration response');}
    catch(e){console.warn('[Sakhi] ElevenLabs route unavailable; using device voice:',e.message);lastError={kind:'PREMIUM_FALLBACK',message:e.message};}
  }
  return browserSpeak(text,false);
}
async function speak(text){
  if(!enabled)throw fault('DISABLED','Spoken guidance is turned off in Parent Settings.');
  lastText=String(text||'');return speakText(lastText);
}
