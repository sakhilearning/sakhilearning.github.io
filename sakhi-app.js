...
async function ensureVoiceReady(){
  ...
  setVoicePreparation(true,'Downloading Sakhi’s gentle voice once…',st.naturalProgress||2);
  var timer=setInterval(function(){var s=Audio.status(),p=s.naturalProgress||2;setVoicePreparation(true,p>=100?'Sakhi is ready!':'Preparing the real Sakhi voice · '+p+'%',p);},350);
...
async function renderParent(){
  ...
  '<section class="parent-card parent-section"><h2>Audio readiness</h2><p class="audio-badge '+(audio.complete?'good':'warn')+'">Verified isolated phonemes: '+audio.present+' / '+audio.required+'</p><p class="muted">Narration uses only the free Kokoro <b>af_heart</b> voice locally, at a calm child-friendly pace. It reads the task and available choices. The bundled single-thread player keeps iPad audio active without paid credits. Browser and robotic device voices remain disabled.</p><div class="audio-tools"><button class="soft" id="testVoiceBtn">🔊 Test Sakhi voice</button><span class="muted" id="audioStatusText"></span></div></section>'+
...
function speak(){var pill=$('#voicePill');if(pill)pill.textContent='✨ Sakhi voice';Audio.narrate(q()).catch(function(e){if(e.kind!=='DISABLED'){console.warn('[Sakhi voice]',e);if(pill)pill.textContent='🔊 Hear again';toast('Sakhi voice could not play. Tap Hear again.');}});}
...
async function boot(){
  try{await Cur.load();Prog.load();Audio.setEnabled(Prog.load().settings.voice!==false);bind();renderHome();renderStats();if(Audio.isEnabled()&&Audio.warm)Audio.warm().catch(function(e){console.warn('[Sakhi] Kokoro preload',e&&e.message||e);});window.__SAKHI_BOOTED=true;document.documentElement.classList.add('sakhi-ready');if(Cloud&&Cloud.probe)Cloud.probe().catch(function(e){console.warn('Cloud probe',e.message);});if('serviceWorker'in navigator&&location.protocol!=='file:'){navigator.serviceWorker.register('./sw.js?v=3.3.0',{updateViaCache:'none'}).then(function(r){r.update().catch(function(){});}).catch(function(e){console.warn('SW',e.message);});}}
