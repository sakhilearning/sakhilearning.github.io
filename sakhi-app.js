...
async function ensureVoiceReady(){
  ...
  setVoicePreparation(true,st.appleMobile?'Connecting to Sakhi’s Kokoro voice…':'Downloading Sakhi’s gentle voice once…',st.naturalProgress||2);
  var timer=setInterval(function(){var s=Audio.status(),p=s.naturalProgress||2;setVoicePreparation(true,p>=100?'Sakhi is ready!':(s.appleMobile?'Preparing Kokoro af_heart · '+p+'%':'Preparing the real Sakhi voice · '+p+'%'),p);},350);
...
async function renderParent(){
  ...
  '<section class="parent-card parent-section"><h2>Audio readiness</h2><p class="audio-badge '+(audio.complete?'good':'warn')+'">Verified isolated phonemes: '+audio.present+' / '+audio.required+'</p><p class="muted">Narration is free and reads the full task and every available choice at a calm child-friendly pace. Every device uses Kokoro <b>af_heart</b>: locally on desktop and securely streamed from Sakhi’s server on iPad and iPhone. No robotic browser voice and no paid speech credits.</p><div class="audio-tools"><button class="soft" id="testVoiceBtn">🔊 Test Sakhi voice</button><span class="muted" id="audioStatusText"></span></div></section>'+
