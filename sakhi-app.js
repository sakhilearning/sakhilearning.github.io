...
async function ensureVoiceReady(){
  ...
  setVoicePreparation(true,st.appleMobile?'Preparing Sakhi’s gentle iPad voice…':'Downloading Sakhi’s gentle voice once…',st.naturalProgress||2);
  var timer=setInterval(function(){var s=Audio.status(),p=s.naturalProgress||2;setVoicePreparation(true,p>=100?'Sakhi is ready!':(s.appleMobile?'Preparing the iPad voice…':'Preparing the real Sakhi voice · '+p+'%'),p);},350);
...
async function renderParent(){
  ...
  '<section class="parent-card parent-section"><h2>Audio readiness</h2><p class="audio-badge '+(audio.complete?'good':'warn')+'">Verified isolated phonemes: '+audio.present+' / '+audio.required+'</p><p class="muted">Narration is free and reads the full task and available choices at a calm child-friendly pace. Mac and desktop use Kokoro <b>af_heart</b>; iPad and iPhone use the best available gentle Apple English voice because the Kokoro model is not stable in iOS browsers.</p><div class="audio-tools"><button class="soft" id="testVoiceBtn">🔊 Test Sakhi voice</button><span class="muted" id="audioStatusText"></span></div></section>'+
