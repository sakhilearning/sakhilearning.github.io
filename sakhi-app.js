...
async function renderParent(){
  ...
  '<section class="parent-card parent-section"><h2>Audio readiness</h2><p class="audio-badge '+(audio.complete?'good':'warn')+'">Verified isolated phonemes: '+audio.present+' / '+audio.required+'</p><p class="muted">Sakhi uses the free Kokoro <b>af_heart</b> voice locally, with no credits required. On the first use, a gentle on-device female voice speaks immediately while Kokoro finishes its one-time download. Isolated phonemes still use verified recordings only.</p><div class="audio-tools"><button class="soft" id="testVoiceBtn">🔊 Test Sakhi voice</button><span class="muted" id="audioStatusText"></span></div></section>'+
  ...
  if($('#testVoiceBtn'))$('#testVoiceBtn').onclick=async function(){var out=$('#audioStatusText');out.textContent='Starting sound…';try{await Audio.unlock();await Audio.speak('Hello! Sakhi voice is ready for our learning adventure.');var st=Audio.status();out.textContent=st.provider==='kokoro'?'Free local Sakhi voice is ready.':'Sound is working. Free Sakhi voice is preparing locally'+(st.naturalProgress?' · '+st.naturalProgress+'%':'')+'.';}catch(e){out.textContent='Tap Test Sakhi voice once more. '+(e.message||'');}};
...
function speak(){var pill=$('#voicePill');if(pill)pill.textContent='🔊 Starting voice…';Audio.narrate(q()).then(function(){if(pill){var st=Audio.status();pill.textContent=st.provider==='kokoro'?'✨ Sakhi voice':'🔊 Sakhi voice · warming up';}}).catch(function(e){if(e.kind!=='DISABLED'){console.warn('[Sakhi voice]',e);if(pill)pill.textContent='🔊 Tap Hear again';toast('Tap Hear again once to start the sound.');}});}
