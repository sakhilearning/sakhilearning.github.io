...
function show(v){Audio.stopAll();view=v;document.querySelectorAll('.view').forEach(function(n){n.classList.toggle('active',n.dataset.view===v);});document.querySelectorAll('[data-nav]').forEach(function(b){b.classList.toggle('active',b.dataset.nav===v);});render();window.scrollTo(0,0);}
...
function bind(){
  document.querySelectorAll('[data-nav]').forEach(function(b){b.onclick=function(){show(b.dataset.nav);};});$('#startBtn').onclick=start;$('#quitBtn').onclick=function(){if(session)Prog.endSession(session.session_id,'LEFT_EARLY');session=null;show('home');};$('#hearBtn').onclick=function(){Audio.repeat().catch(function(){toast('Voice unavailable right now.');});};$('#hintBtn').onclick=hint;$('#checkBtn').onclick=check;$('#continueBtn').onclick=continueAfter;$('#gateBtn').onclick=function(){if($('#gateAnswer').value.trim()==='11'){parentOpen=true;renderParent();}else toast('Try the grown-up question again.');};Audio.onFault(function(e){console.warn('[Sakhi audio]',e.kind,e.message);});
}
