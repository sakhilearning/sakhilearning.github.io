...
function bind(){
  document.querySelectorAll('[data-nav]').forEach(function(b){b.onclick=function(){show(b.dataset.nav);};});$('#startBtn').onclick=start;$('#modeUnicorn').onclick=function(){adventureMode='unicorn';localStorage.setItem('sakhi_adventure_mode',adventureMode);renderHome();};$('#modePrincess').onclick=function(){adventureMode='princess';localStorage.setItem('sakhi_adventure_mode',adventureMode);renderHome();};$('#quitBtn').onclick=function(){if(session)Prog.endSession(session.session_id,'LEFT_EARLY');session=null;show('home');};$('#hearBtn').onclick=async function(){try{await Audio.unlock();await Audio.narrate(q());var st=Audio.status();var pill=$('#voicePill');if(pill)pill.textContent=st.provider==='elevenlabs'?'✨ Sakhi voice':'🔊 Device voice';}catch(e){console.warn('[Sakhi repeat]',e);toast('Voice could not play. A grown-up can check Audio in Parents.');}};$('#hintBtn').onclick=hint;$('#checkBtn').onclick=check;$('#continueBtn').onclick=continueAfter;$('#gateBtn').onclick=function(){if($('#gateAnswer').value.trim()==='11'){parentOpen=true;renderParent();}else toast('Try the grown-up question again.');};Audio.onFault(function(e){console.warn('[Sakhi audio]',e.kind,e.message);});
}
