...
async function renderParent(){
  ...
  '<section class="parent-card parent-section"><h2>Audio readiness</h2><p class="audio-badge '+(audio.complete?'good':'warn')+'">Verified isolated phonemes: '+audio.present+' / '+audio.required+'</p><p class="muted">Normal instructions use only the bundled gentle Sakhi voice. Browser speech is disabled so a robotic AI voice cannot replace it. Isolated phonemes never use TTS.</p><div class="audio-tools"><button class="soft" id="testVoiceBtn">🔊 Test Sakhi voice</button><span class="muted" id="audioStatusText"></span></div></section>'+
  ...
  if($('#testVoiceBtn'))$('#testVoiceBtn').onclick=async function(){var out=$('#audioStatusText');try{await Audio.unlock();await Audio.speak('Hello! Sakhi voice is ready for our learning adventure.');out.textContent='Playing with the bundled Sakhi voice.';}catch(e){out.textContent='Bundled gentle Sakhi narration is not installed in this build.';}};
