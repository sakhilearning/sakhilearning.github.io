...
async function renderParent(){
  ...
  '<section class="parent-card parent-section"><h2>Audio readiness</h2><p class="audio-badge '+(audio.complete?'good':'warn')+'">Verified isolated phonemes: '+audio.present+' / '+audio.required+'</p><p class="muted">Normal instructions use the natural Sakhi voice route. Device speech is available only when a grown-up explicitly tests the fallback. Isolated phonemes never use TTS.</p><div class="audio-tools"><button class="soft" id="testVoiceBtn">🔊 Test Sakhi voice (fallback)</button><span class="muted" id="audioStatusText"></span></div></section>'+
  ...
  if($('#testVoiceBtn'))$('#testVoiceBtn').onclick=async function(){var out=$('#audioStatusText');try{await Audio.unlock();await Audio.speak('Hello! Sakhi voice is ready for our learning adventure.');var st=Audio.status();out.textContent='Playing with '+(st.provider==='elevenlabs'?'Sakhi natural voice':'device fallback')+'.';}catch(e){out.textContent='Natural Sakhi voice is not configured for this build yet.';}};
...
function bind(){
  document.querySelectorAll('[data-nav]').forEach(function(b){b.onclick=function(){show(b.dataset.nav);};});document.addEventListener('click',function(e){var card=e.target.closest&&e.target.closest('[data-trail-domain]');if(card)startTrail(card.dataset.trailDomain);});document.addEventListener('keydown',function(e){var card=e.target.closest&&e.target.closest('[data-trail-domain]');if(card&&(e.key==='Enter'||e.key===' ')){e.preventDefault();startTrail(card.dataset.trailDomain);}});$('#startBtn').onclick=start;$('#modeUnicorn').onclick=function(){adventureMode='unicorn';localStorage.setItem('sakhi_adventure_mode',adventureMode);renderHome();};$('#modePrincess').onclick=function(){adventureMode='princess';localStorage.setItem('sakhi_adventure_mode',adventureMode);renderHome();};$('#quitBtn').onclick=function(){if(session)Prog.endSession(session.session_id,'LEFT_EARLY');session=null;show('home');};$('#hearBtn').onclick=async function(){try{await Audio.unlock();await Audio.narrate(q(),{allowBrowserFallback:true});var st=Audio.status();var pill=$('#voicePill');if(pill)pill.textContent=st.provider==='elevenlabs'?'✨ Sakhi voice':'🔊 Device fallback';}catch(e){console.warn('[Sakhi repeat]',e);toast('Voice could not play. A grown-up can check Audio in Parents.');}};$('#hintBtn').onclick=hint;$('#checkBtn').onclick=check;$('#continueBtn').onclick=continueAfter;$('#gateBtn').onclick=function(){var answer=String($('#gateAnswer').value||'').replace(/\s/g,'');if(answer==='071621'){parentOpen=true;$('#gateAnswer').value='';renderParent();}else toast('Try the grown-up question again. That passcode did not match.');};Audio.onFault(function(e){console.warn('[Sakhi audio]',e.kind,e.message);});
}
