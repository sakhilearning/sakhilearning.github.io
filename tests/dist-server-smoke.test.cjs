...
(async () => {
  ...
  try {
    ...
    if (!sw.includes('sakhi-v4-4.2.0') || sw.includes('client.navigate') || sw.includes('clients.matchAll')) throw new Error('Built service worker must update without reload loops');
