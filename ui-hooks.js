...
function on(event,handler){if(typeof handler!=='function')throw new TypeError('UIHooks handler must be a function');const set=listeners.get(event)||new Set();set.add(handler);listeners.set(event,set);return()=>set.delete(handler);}
...
function wrap(name,beforeEvent,afterEvent){const base=window[name];if(typeof base!=='function')return;window[name]=function(...args){const payload={name,args};if(beforeEvent&&!allow(beforeEvent,payload))return false;const result=base.apply(this,args);if(afterEvent)emit(afterEvent,{...payload,result});return result;};}
