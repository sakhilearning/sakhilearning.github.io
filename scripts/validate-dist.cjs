...
if(!/3\.8\.0/.test(html))throw new Error('3.8.0 build marker missing');
...
if(!/KokoroTTS\.from_pretrained/.test(html)||!/af_heart/.test(html)||!/NATURAL_SPEED=\.86/.test(html)||!/describeQuestion/.test(html)||!/startKeepAlive/.test(html)||!/prepare:loadNaturalVoice/.test(html)||!/playWithHtmlAudio/.test(html)||!/voicePrepare/.test(html))throw new Error('Detailed iPad-safe Kokoro readiness gate missing from deployment');
if(!/SAKHI_TTS_URL[\s\S]*kokoro-server/.test(html)||!/naturalMode:APPLE_MOBILE\?'kokoro-server':'kokoro-local'/.test(html))throw new Error('iOS server-side Kokoro path missing from deployment');
...
console.log(' - 3.8.0 instant startup, tablet-safe transitions, and contextual scenes embedded');
