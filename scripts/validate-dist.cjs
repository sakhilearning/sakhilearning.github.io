...
if(!/3\.4\.0/.test(html))throw new Error('3.4.0 build marker missing');
...
if(!/APPLE_MOBILE[\s\S]*speechSynthesis/.test(html)||!/naturalMode:APPLE_MOBILE\?'apple-system':'kokoro-local'/.test(html))throw new Error('iOS-only no-cost voice fallback missing from deployment');
...
console.log(' - 3.4.0 screenshot-matched cinematic themes embedded');
