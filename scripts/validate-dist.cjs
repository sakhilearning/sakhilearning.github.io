...
if(!/3\.2\.0/.test(html))throw new Error('3.2.0 build marker missing');
...
if(!/KokoroTTS\.from_pretrained/.test(html)||!/af_heart/.test(html)||!/prepare:loadNaturalVoice/.test(html)||!/playWithHtmlAudio/.test(html)||!/voicePrepare/.test(html))throw new Error('Kokoro-only readiness gate missing from deployment');
...
console.log(' - 3.2.0 screenshot-matched cinematic themes embedded');
