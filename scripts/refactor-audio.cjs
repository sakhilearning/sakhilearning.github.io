...
function migrateBlock(source,from,to,label){
  if(source.includes(from))return source.replace(from,to);
  if(source.includes(to))return source;
  throw new Error(`Could not find old or new ${label}`);
}

let kid=fs.readFileSync('kid-upgrade.js','utf8');
const oldKidAudio=`const KID_AUDIO={
...
const newKidAudio=`const KID_AUDIO={
...
kid=migrateBlock(kid,oldKidAudio,newKidAudio,'KID_AUDIO metadata');

const oldKidSpeech=`function safeSpeak(text){
...
const newKidSpeech=`function speech(){
...
window.playLetterSound=playLetterSound;`;
kid=migrateBlock(kid,oldKidSpeech,newKidSpeech,'kid speech owner');
...
engine=migrateBlock(engine,oldEngineSpeech,newEngineSpeech,'interaction-engine speech owner');
