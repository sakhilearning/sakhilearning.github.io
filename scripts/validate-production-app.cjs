...
if(!/KokoroTTS\.from_pretrained/.test(audio)||!/af_heart/.test(audio)||!/browserSpeak/.test(audio)||!/playWithHtmlAudio/.test(audio)||!/narrate:/.test(audio))throw new Error('Free resilient audio hardening missing');
...
console.log('Sakhi V3.1.3 source validation passed:');
...
console.log(' - free local Kokoro playback, no-silence fallback, themed story cues and tappable learning objects present');
