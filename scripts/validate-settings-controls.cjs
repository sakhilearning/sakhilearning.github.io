...
for(const token of ['SettingsService','adaptiveProfile.settings',"session_length:'AUTO'","preferred_adventure:'AUTO'",'movement_activities:true','reduced_motion:false','fitActivities','persist'])if(!settings.includes(token))errors.push(`settings service missing ${token}`);
if(/speech\[name\]\s*=|Object\.defineProperty\(window\s*,\s*['"]ProgressService['"]/.test(settings))errors.push('settings service must not wrap SpeechService or ProgressService; those services have single authoritative owners');
...
console.log('Settings/control audit valid: one settings source, no service wrappers, persistent profile state, curated valid adventures, working session/audio/movement/reduced-motion effects, and parent QA controls.');
