...
'use strict';
var data=null, byId={};
async function load(){ if(data) return data; var r=await fetch('./data/curriculum-v3.json',{cache:'no-store'}); if(!r.ok) throw new Error('Curriculum failed to load'); data=await r.json(); byId={}; data.skills.forEach(function(s){byId[s.skill_id]=s;}); return data; }
...
function prerequisites(id){var s=skill(id);return s?s.prerequisites.slice():[];}
