...
function evidence(id){var s=load();if(!s.skills[id])s.skills[id]={skill_id:id,mastery_state:'NOT_INTRODUCED',attempt_count:0,objective_attempts:0,correct_count:0,independent_correct_count:0,hints_used:0,difficulty_level:1,last_practiced:null,next_review:null};return s.skills[id];}
...
function completeActivity(payload){var s=load(),a=payload.activity,sk=SakhiCurriculum.skill(a.skill_id);if(!sk)throw new Error('Unknown skill');var e=evidence(a.skill_id),answers=payload.answers||[],objective=a.evidence_mode!=='practice';var correct=answers.filter(function(x){return x.correct;}).length,ind=answers.filter(function(x){return x.correct&&!x.hintsUsed;}).length;
 e.attempt_count+=1;e.last_practiced=new Date().toISOString();if(objective){e.objective_attempts+=1;if(correct===answers.length)e.correct_count+=1;if(ind===answers.length)e.independent_correct_count+=1;e.hints_used+=answers.reduce(function(n,x){return n+(x.hintsUsed||0);},0);recompute(e,sk.mastery_criteria);var d=new Date();d.setDate(d.getDate()+reviewDays(e,sk.review_policy));e.next_review=d.toISOString();}else{if(e.mastery_state==='NOT_INTRODUCED')e.mastery_state='LEARNING';}
 var reward={id:(crypto.randomUUID?crypto.randomUUID():String(Date.now())+Math.random()),at:new Date().toISOString(),skill_id:a.skill_id,amount:objective?3:1,label:'Trail Stars'};s.rewards.push(reward);var attempt={id:reward.id,at:reward.at,skill_id:a.skill_id,domain_id:a.domain_id,objective:objective,correct_questions:correct,total_questions:answers.length,independent_questions:ind,hints:answers.reduce(function(n,x){return n+(x.hintsUsed||0);},0),band:a.band};s.attempts.push(attempt);persist();if(window.SakhiCloud)SakhiCloud.enqueue([attempt]);return{skill_id:a.skill_id,accuracy:answers.length?correct/answers.length:1,correct:correct,independent:ind,mastery_state:e.mastery_state,rewards_issued:[reward]};}
...
function endSession(id){var s=load(),x=s.sessions.find(function(v){return v.session_id===id;});if(x){x.ended_at=new Date().toISOString();x.status='COMPLETED';persist();}}
